...
export default class ForceGraph extends Component {
  constructor(props) {
    ...
    this.getParticipantRadius = d3
      .scaleLinear()
      .domain([0, 500000])
      .range([12, 50])

    this.getProposalRadius = d3
      .scaleLinear()
      .domain([0, 50000])
      .range([6, 20])
  ...
  initialize() {
    ...
    simulation = this.simulation = d3
      .forceSimulation(self.graphNodes)
      .force(
        'link',
        d3
          .forceLink(self.graphLinks)
          .id(function(d) {
            return d.id
          })
          .distance(function(d) {
            return self.linkDistance(d)
          })
      )
      ...
      .alphaMin(0)
      .on('tick', function() {
        self.tickActions(self.nodeCollection, self.linkCollection)
      })
  ...
  setupNodes() {
    const self = this
    this.nodeCollection = select(this.node)
      .append('g')
      .attr('class', 'nodes')
      .selectAll('circle')
      ...
      .call(
        d3
          .drag()
          .on('start', function(d) {
            self.dragstarted(self.simulation, d)
          })
          .on('drag', function(d) {
            self.dragged(d)
          })
          .on('end', function(d) {
            self.dragended(self.simulation, d)
          })
      )
      .on('click', function(d) {
        self.nodeClick(d)
  ...
  setupLinks() {
    this.linkCollection = select(this.node)
      .append('g')
      .attr('class', 'links')
      .selectAll('.link')
